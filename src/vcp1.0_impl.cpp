...
bool vcp10_init(char * s)
{
	...
	if (vpinit == 0 )
	{
		if(!Extensions::instance()->isExtensionSupported("GL_NV_vertex_program"))
		{
					...
					!strnicmp(token, "c[", 2)) // the first word is of the form "c[xx]", so its a constant.
				{
					...
					{ // this is a TrackMatrix directive
						...
						else
						{
							...
							{
								sprintf(error, 
									"error at line %d \n\"%s\"\n\tExtensions::instance()->glTrackMatrixNV address must be modulo 4\n",
									iLineCount, token);
								errors.set(error);
							}
							...
							else
							{
								...
								{
									sprintf(error, 
										"error at line %d \n\"%s\"\n\tInvalid Matrix parameter in Extensions::instance()->glTrackMatrixNV.\n",
										iLineCount, token);
									errors.set(error);
								}
								...
								{
									sprintf(error, 
										"error at line %d \n\"%s\"\n\tInvalid Transform parameter in Extensions::instance()->glTrackMatrixNV\n",
										iLineCount, token);
									errors.set(error);
								}
								else
								{
									...
									Extensions::instance()->glTrackMatrixNV(target, iConstID, GL_NONE, GL_IDENTITY_NV);
									
									// tell GL the matrix to track
									Extensions::instance()->glTrackMatrixNV(target, iConstID, iMatrixID, iTransformID);         
								}
							}
						}
					}
					...
					{  
						...
						else 
						{
							...
							Extensions::instance()->glTrackMatrixNV(target, iConstID - (iConstID % 4), GL_NONE, GL_IDENTITY_NV);
							
							// tell GL the constant values
							Extensions::instance()->glProgramParameter4fvNV(target, iConstID, xyzw);
						}
					}
        }
      }
