...
void GeneralCombinersStruct::Validate(int numConsts, ConstColorStruct *pcc)
{
	...
	if (localConsts > 0)
		if (NULL == Extensions::instance()->glCombinerStageParameterfvNV)
...
{
	Extensions::instance()->glCombinerParameteriNV(GL_NUM_GENERAL_COMBINERS_NV, num);
	int i;
	for (i = 0; i < num; i++)
		general[i].Invoke(i);
...
void GeneralCombinerStruct::Invoke(int stage)
{
	...
	if (NULL != Extensions::instance()->glCombinerStageParameterfvNV)
		for (i = 0; i < numConsts; i++)
			Extensions::instance()->glCombinerStageParameterfvNV(GL_COMBINER0_NV + stage, cc[i].reg.bits.name, &(cc[i].v[0]));
...
void GeneralFunctionStruct::Invoke(int stage, int portion, BiasScaleEnum bs)
{
	...
	Extensions::instance()->glCombinerInputNV(GL_COMBINER0_NV + stage,
		portionEnum,
		GL_VARIABLE_A_NV,
	...
	Extensions::instance()->glCombinerOutputNV(GL_COMBINER0_NV + stage,
		portionEnum,
